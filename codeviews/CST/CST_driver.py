...
class CSTGraph:
    def __init__(self, src_language, src_code, root_node):
        ...
        postprocessor.write_to_dot(self.graph, "./output_graphs/CST_output.dot")
    ...
    def create_CST_id(self, root_node, CST_index, CST_id):
        ...
        CST_id[0] += 1
        CST_index[(root_node.start_point, root_node.end_point, root_node.type)] = current_node_id
    ...
    def get_CST_nodes(self, root_node, CST, CST_index):
        current_node_id = CST_index[(root_node.start_point,root_node.end_point, root_node.type)]
        label = str(root_node.start_point[0]) + "_" + root_node.type 
        CST.add_node(current_node_id,  label = label,  shape = 'box', style = 'rounded, filled', fillcolor = '#BFE6D3', color = 'white')
        for child in root_node.children:
